...
import type { Account, InstagramPostOptions, MediaView } from '@/types/compose';
...
export function InstagramCoverPicker(props: Props) {
    ...
    return (
        <CoverPicker
            key={`${props.postId ?? 'new'}:${props.account.id}`}
            {...props}
        />
    );
}
...
function CoverPicker({
    ...
    const selectedId = options?.cover_media_id ?? null;
    ...
    async function loadPage(cursor: string | null = null) {
        ...
        try {
            const data = await galleryHttp.get(
                InstagramCoverController.index(postId, {
                    query: {
    ...
    }, [postId, selectedId, open, revision]);

    async function uploadCover(file: File) {
        if (!postId || !canChoose || uploading) return;
        const fileError = instagramCoverFileError(file);
        if (fileError) {
            setError(fileError);
            return;
        }
        ...
        try {
            ...
            if (selectionAllowed.current) {
                ...
                onChange({ cover_media_id: media.id });
            } else {
                setError(
    ...
    return (
        <section
            ...
            aria-label={`Instagram cover for ${account.handle}`}
        >
            <div>
                <h3 className="text-sm font-medium">Instagram Reel cover</h3>
                ...
                <div className="flex items-center gap-3">
                    {selected ? (
                        <img
                            ...
                            alt={
                                selected.alt_text ||
                                'Selected Instagram Reel cover'
                            }
                            className="h-28 w-20 rounded-md border bg-muted object-contain"
                        />
                    ) : (
                        <p className="text-sm text-muted-foreground">
                            {loading
            ...
            {!canChoose && selectedId && (
                <p role="alert" className="text-sm text-destructive">
                    This cover requires exactly one video in a Reel or feed
                    post. Change the format or media, or remove the cover before
                    publishing.
                </p>
            )}
            {!postId ? (
                ...
                <div className="flex flex-wrap gap-2">
                    ...
                    <input
                        ...
                        aria-label="Upload Instagram cover image"
                        className="hidden"
                        disabled={!canChoose || uploading}
                        onChange={(event) => {
            ...
            {open && (
                <div className="space-y-3">
                    <p className="text-xs text-muted-foreground">
                        Workspace JPEG and PNG images up to 8 MB. Choose a cover
                    ...
                    <div className="grid grid-cols-3 gap-2 sm:grid-cols-4">
                        {items.map((item, index) => (
                            <button
                                ...
                                disabled={!canChoose || uploading}
                                onClick={() =>
                                    onChange({ cover_media_id: item.id })
                                }
                            >
                                <img
                                    src={item.url}
